...
## TO DO
-   L 271-272 doesn't make sense? "below we do both with row sums as
proportions of matrix sum as weights"
-   Figure 10, 11 - sometimes the plots dimensions are horrible (other times they are fine), might be to do with coord_equal() and stocastic nature of Rtsne? 
-   Add caption to figure 19
...
## plots need tidying
### legend - cl/colour = true/false native vegertation > 0.2
### plot_wgt = physa abundance??
...
```{r fig.cap = "UMAP for the Perry et al. data with different hyper-parameter values (see code block above)", fig.dim = c(8, 4)}
fig12
```
...
```{r plotFullSel, fig.cap = paste("dbRDA including all hypothesised predictors, Adjusted R2 = ", full_mod_dbrda_r2, " (a) ", "and a subset based on model selection, Adjusted R^2^ = ", sel_mod_dbrda_r2, " (b)", sep = ""), echo = FALSE}
fig17
```
...
```{r echo = FALSE, fig.cap = "ADD FIGURE CAPTION", fig.dim = c(8, 4), warning = FALSE, message = FALSE}
fig19
```
...
```{r mdsTRaj, message = FALSE, warning = FALSE, fig.cap = "ADD CAPTION", fig.dim = c(8, 4)}
...
```{r, fig.cap = "ADD CAPTION", fig.dim = c(8, 4)}
...
(mnean = variance) distribution.

```{r invertDownWeight, message = FALSE, warning = FALSE}
...
```{r echo = FALSE, fig.cap = "ADD CAPTION", fig.dim = c(5,4)}
fig24
```

```{r echo = FALSE, fig.cap = "ADD CAPTION", fig.dim = c(8,4)}
fig25
```
