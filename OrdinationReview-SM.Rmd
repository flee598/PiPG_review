...
pacman::p_load(tidyverse, broom, knitr, janitor, ggfortify, patchwork,
               mdthemes, vegan, analogue, ggvegan, mvabund, Rtsne, umap, devtools)
```
...
For tSNE the key hyperparameter is _perplexity_, which controls the balance between local and global structure (by, in essence, determining each point's neighbourhood).  The Rtsne help recommends that $3 * perpexity < nrow - 1$ (the default is 30, but that is too high for the Lee et al. invertebrate data). 
...
```{r plotivTSNE, fig.cap = "tSNE for the Lee et al. data with different perplecities and using distance matrices."}
tsne3.gg <- plot.tsne.gg(invert_tsne_p3, invert_sites)
tsne6.gg <- plot.tsne.gg(invert_tsne_p6, invert_sites)
tsned3.gg <- plot.tsne.gg(invert_tsne_bc_p3, invert_sites)
tsned6.gg <- plot.tsne.gg(invert_tsne_bc_p6, invert_sites)
...
```{r plotAoteaTSNE, fig.cap = "tSNE for the Perry et al. data with different perplexities and using distance matrices; colours represent geographic locations."}

tsne_ao_p25.gg <- plot.tsne.gg(aotea_tsne_p25, labels_txt = FALSE, clusters = aotea_sites_nd)
tsne_ao_p50.gg <- plot.tsne.gg(aotea_tsne_p50, labels_txt = FALSE, clusters = aotea_sites_nd)
tsne_ao_d_p25.gg <- plot.tsne.gg(aotea_tsne_bc_p25, labels_txt = FALSE, clusters = aotea_sites_nd)
tsne_ao_d_p50.gg <- plot.tsne.gg(aotea_tsne_bc_p50, labels_txt = FALSE, clusters = aotea_sites_nd)

(tsne_ao_p25.gg + tsne_ao_p50.gg + tsne_ao_d_p25.gg + tsne_ao_d_p50.gg) +
...
```{r plotUmapAotea, fig.cap = "UMAP for the Perry et al. data with different hyper-parameter values; colours represent geographic locations."}
aotea_umap  <- plot.umap.gg(aotea_umap, labels_txt = FALSE, clusters = aotea_site$code)
aotea_umap_nn30  <- plot.umap.gg(aotea_umap_nn30, labels_txt = FALSE, clusters = aotea_site$code)
aotea_umap_md05 <- plot.umap.gg(aotea_umap_md05, labels_txt = FALSE, clusters = aotea_site$code)


(aotea_umap + aotea_umap_nn30 + aotea_umap_md05) +
...
aotea_pw_adonis <- pairwise.adonis2(aotea_dist ~ aotea_site$code, permutations = n_perm, parallel = n_cores)
...
These two plots show the obserrved (red line) test statistic and the distribution of the simulated values -- in both cases suggesting that there is a strong location effect of site (in this case due to successional differences).
