...
Commented R code for analyses described in Brock et al. *Methods of classification and ordination in biogeography I - distance-based (algorithm-based) methods* in PiPG.Below is information on the operating system, version of R and package versions we used during the creation of this document.
...
al. largely using data from Lee et al.
...
(ecological) data using the Lee et al. 2018 data. 
...
quality. The vectors suggest that dissolved inorganic nitrogen (DIN) and
...
One issue with fitting vectors is that they can not depict non-linear
...
plot_pca_k5 <- plot.ordsurf.gg(invert_mds, pca_surf_k5, lbl = "PCA comp 1", knts = 5)
plot_din_k10 <- plot.ordsurf.gg(invert_mds, din_surf_k10, lbl = "DIN", knts = 10, comb = T)
plot_pca_k10 <- plot.ordsurf.gg(invert_mds, pca_surf_k10, lbl = "PCA comp 1", knts = 10)
...
```{r invertSurface, fig.cap = "Dissolved inorganic nitrogen (DIN) fit as a surface to the nMDS with different paramertisations of *k* (a, c). PCA component 1 fit as a surface to the nMDS with different paramertisations of *k* (b, d).", fig.dim = c(8, 8), message = FALSE, warning = FALSE, echo = FALSE}
fig5
```

### Perry et al. vegetation data

Now we'll look at an nMDS of the Aotea vegetation data. Unlike the Lee et al.
...
```{r fig.cap = "canonical correspondence analysis of freshwater invertebrate and environemntal data using all (a) and a subset (b) of environmental predictors.", fig.dim = c(8, 6), echo = FALSE}
fig15
```
...
In tests of species-environment relationships, spatial autocorrelation is
known to inflate the type-I error rate and make the tests of significance
invalid [@PeresMEM]. To account for this, we can first test for spatial autocorrelation,
(using Moran's eigenvector maps). We will do this using the Lee et al data.

As an alternative to computing univariate statistics to check for spatial
autocorrelation, we can summarise data by multivariate methods and then detect
spatial structures using the output of the analysis [@PeresMEM]. For example, using
centred principal component analysis on the invertebrate abundance data:

Note we have used the default settings for all of the functions below. See the `adespatial` [vignette](https://cran.r-project.org/web/packages/adespatial/vignettes/tutorial.html) for more details for parameter settings for the functions.
...
# invertebrate data
invert_mtx <- as.matrix(inverts)

# Generating weighted graph of site coordinates 
coord_graph <- graph2nb(gabrielneigh(xy_coords), sym = TRUE)

# get distances between points
coord_dist <- nbdists(coord_graph, xy_coords)
dists_1 <- lapply(coord_dist, function(x) 1 - x/max(dist(xy_coords)))

# add spatial (distances) to neighbours list
coord_graph_dist <- nb2listw(coord_graph, glist = dists_1)

# centred principal component analysis on the abundance data
pca_inv <- dudi.pca(invert_mtx, scale = FALSE, scannf = FALSE, nf = 2)

# global test for autocorrelation and forward selection of MEMs
mem_inv <- mem.select(pca_inv$tab, listw = coord_graph_dist)

# global test result 
mem_inv$global.test

# selected MEMs summary
mem_inv$summary
...
The subset of selected MEMs above can then be used in subsequent analysis e.g. Redundancy analysis.
...
# GP has coded various helpers for ggplot2
...
permutation-based (meaning we can use parallelisation to speed things
up) and operate on the dissimilarity matrix rather than an ordination.
...
```{r fig.cap = "Observed (red line) ANOSIM and PERMANOVA test statistic and the distribution of the simulated values – in both cases suggesting that there is a strong location effect of site", fig.dim = c(8, 4), echo = FALSE, message = FALSE}
fig22
```


```{r permuTablesOp, fig.cap = "Ordination illustrating the dispersion of species across sites.", fig.dim = c(6,6), warning = FALSE, message = FALSE}
